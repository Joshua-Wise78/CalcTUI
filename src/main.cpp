...
std::string evaluate(const std::string &expression) {
  ...
  if (ss >> left_operand >> math_operator >> right_operand) {
    switch (math_operator) {
    case '+':
      return std::to_string(left_operand + right_operand);
    case '-':
      return std::to_string(left_operand - right_operand);
    case '*':
      return std::to_string(left_operand * right_operand);
...
int main() {
  ...
  auto plus = Button("+", [&] { on_click("+"); });
  auto minus = Button("-", [&] { on_click("-"); });
  auto divide = Button("/", [&] { on_click("/"); });
  auto multiply = Button("*", [&] { on_click("*"); });

  auto equals = Button("=", [&] { value = evaluate(value); });
  auto clear = Button("C", [&] { value = ""; });
  ...
  auto row4 = Container::Horizontal({plus, minus, divide, multiply});
  auto row5 = Container::Horizontal({clear, equals});

  auto keypad = Container::Vertical({row1, row2, row3, row4, row5});
  ...
  auto calculator_ui = Renderer(keypad, [&] {
    return vbox({
        ...
        hbox({
            vbox({plus->Render() | flex}) | border,
            vbox({minus->Render() | flex}) | border,
            vbox({divide->Render() | flex}) | border,
            vbox({multiply->Render() | flex}) | border,
        }),
        hbox({
            vbox({clear->Render() | flex}) | border,
            vbox({equals->Render() | flex}) | border,
        }),
